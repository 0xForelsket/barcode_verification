...
def check_pin_rate_limit(ip: str) -> tuple[bool, str]:
    ...
    if len(attempts) >= MAX_PIN_ATTEMPTS:
        ...
        return (
            False,
            f"Too many PIN attempts. Try again in {minutes_remaining} minutes.",
        )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(
        status_code=422, content={"error": "Invalid input data", "details": errors}
    )
...
async def start_job(request: JobStartRequest, session: Session = Depends(get_session)):
    logger.info(
        f"Job start request: job_id={request.job_id}, barcode={request.expected_barcode[:20]}..."
    )
    ...
    if not request.expected_barcode.strip():
        ...
        return JSONResponse(
            status_code=400, content={"error": "Expected barcode is required"}
        )
        ...
        try:
            ...
            if active:
                ...
                return JSONResponse(
                    status_code=400,
                    content={
                        "error": f"A job is already active: {active.job_id}. End it first."
                    },
                )
            ...
            if "database is locked" in str(e) and attempt < max_retries - 1:
                ...
                logger.error(
                    f"Failed to start job after {attempt + 1} attempts: {e}",
                    exc_info=True,
                )
                session.rollback()
                return JSONResponse(
...
async def verify_pin(req_data: JobEndRequest, request: Request):
    ...
    if not success:
        ...
        return JSONResponse(
            status_code=403, content={"error": "Invalid supervisor PIN"}
        )
...
async def end_job(
    ...
    try:
        ...
        await notify_clients(
            "job_ended", {"job": job_read.model_dump(), "shift": shift.model_dump()}
        )
...
async def process_scan(request: ScanRequest, session: Session = Depends(get_session)):
    ...
    try:
        ...
        # CHECK LOCK STATE
        if job.is_locked:
            logger.warning(f"Scan rejected: Job {job.job_id} is locked")
            return JSONResponse(
                status_code=423,
                content={"error": "Line is locked. Supervisor PIN required."},
            )

        status = "PASS" if barcode == job.expected_barcode else "FAIL"
        logger.info(
            f"Scan processed: job={job.job_id}, status={status}, barcode={barcode[:20]}..."
        )
        ...
        if status == "PASS":
            ...
            # LOCK ON FAIL
            job.is_locked = True
            logger.warning(f"Scan FAILED. Locking job {job.job_id}")
...
async def monitor(request: Request, session: Session = Depends(get_session)):
    ...
    recent_jobs = session.exec(
        select(Job).order_by(Job.start_time.desc()).limit(10)
    ).all()

    return templates.TemplateResponse(
        request=request,
        name="monitor.html",
        context={"active_job": active_job, "shift": shift, "recent_jobs": recent_jobs},
    )
...
async def history(
    ...
    jobs = session.exec(
        select(Job).order_by(Job.start_time.desc()).offset(offset).limit(limit)
    ).all()
    ...
    return templates.TemplateResponse(
        request=request, name="history.html", context={"jobs": pagination, "page": page}
    )
...
async def export_csv(session: Session = Depends(get_session)):
    ...
    jobs = session.exec(
        select(Job).where(Job.start_time >= cutoff_date).order_by(Job.start_time.desc())
    ).all()
...
async def backup_data(session: Session = Depends(get_session)):
    ...
    if active_job:
        ...
        state["active_job"] = {
            ...
            "end_time": active_job.end_time.isoformat()
            if active_job.end_time
            else None,
            "recent_scans": [
                {
                    "barcode": s.barcode,
